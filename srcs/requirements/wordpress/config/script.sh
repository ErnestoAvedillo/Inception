...
if [ -d ${WP_PATH}"/wp-admin" ]; then
	...
	wp config create  --path="${WP_PATH}" --dbname="${MARIADB_NAME}" --dbuser="${MARIADB_USER}" --dbpass="${MARIADB_USER_PASSWORD}" --dbhost="${MARIADB_HOST}" --allow-root
	wp theme activate twentytwentytwo --path="${WP_PATH}" --allow-root
	chmod -R 777 ${WP_PATH} && chown -R www-data:www-data ${WP_PATH}
fi
